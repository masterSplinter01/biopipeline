...
rule all:
    input: config["output_folder"] + "freq_parser/rare_variations/" + config["read_name"] + ".result.txt", config["output_folder"] + "gatk/rbq/" + "post_recal_data.table"
...
rule create_realign_target:#index file is needed
    input: "reference/Homo_sapiens_assembly38.fasta", config["output_folder"] + "picard-tools/" + config["read_name"] + ".bam", config["output_folder"] + "picard-tools/" + config["read_name"] + ".bai"
    output: config["output_folder"]+"gatk/realign_target/"+config["read_name"]+".intervals"
    shell: "docker run -it --rm -v $(pwd):/root/source phedo/gatk -T RealignerTargetCreator -R {input[0]} -I {input[1]} -o {output}"

rule realign_indels: #index file is needed
    input:  "reference/Homo_sapiens_assembly38.fasta", config["output_folder"] + "picard-tools/" + config["read_name"] + ".bam","reference/Mills_and_1000G_gold_standard.indels.hg38.vcf.gz", config["output_folder"]+"gatk/realign_target/"+config["read_name"]+".intervals", config["output_folder"] + "picard-tools/" + config["read_name"] + ".bai"
    output: config["output_folder"]+"gatk/realign_indels/"+config["read_name"]+"_realigned.bam"
    shell: "docker run -it --rm -v $(pwd):/root/source phedo/gatk -T IndelRealigner -R {input[0]} -I {input[1]} -known {input[2]} -targetIntervals {input[3]} -o {output}"

rule gatk_rbq_analyze: #index file is needed
    input: "reference/Homo_sapiens_assembly38.fasta", config["output_folder"]+"gatk/realign_indels/"+config["read_name"]+"_realigned.bam", "reference/Homo_sapiens_assembly38.dbsnp138.vcf", "reference/Mills_and_1000G_gold_standard.indels.hg38.vcf.gz", config["output_folder"] + "picard-tools/" + config["read_name"] + ".bai"
...
rule gatk_extract_snp:
    input: "reference/Homo_sapiens_assembly38.fasta", config["output_folder"] + "gatk/call_variants/" + config["read_name"] + "_raw_variants.vcf" 
    output: config["output_folder"] + "gatk/extract_snp/" + config["read_name"] + "_raw_snps.vcf"
    shell: "docker run -it --rm -v $(pwd):/root/source phedo/gatk -R {input[0]} -T SelectVariants -V {input[1]} -selectType SNP -o {output}" 

rule gatk_filter_snp:
    input: "reference/Homo_sapiens_assembly38.fasta", config["output_folder"] + "gatk/extract_snp/" + config["read_name"] + "_raw_snps.vcf"
    output: config["output_folder"] + "gatk/filter_snp/" + config["read_name"] + "_filtered_snps.vcf"
    shell:  "docker run -it --rm -v $(pwd):/root/source phedo/gatk -T VariantFiltration -R {input[0]} -V {input[1]} --filterExpression 'QD < 2.0 || FS > 60.0 || MQ < 40.0 || MQRankSum < -12.5 || ReadPosRankSum < -8.0' --filterName 'my_snp_filter' -o {output}"

rule gatk_extract_indels:
    input: "reference/Homo_sapiens_assembly38.fasta", config["output_folder"] + "gatk/call_variants/" + config["read_name"] + "_raw_variants.vcf" 
    output: config["output_folder"] + "gatk/extract_indels/" + config["read_name"] + "_raw_indels.vcf"
    shell: "docker run -it --rm -v $(pwd):/root/source phedo/gatk -R {input[0]} -T SelectVariants -V {input[1]} -selectType INDEL -o {output}"

rule gatk_filter_indels:
    input: "reference/Homo_sapiens_assembly38.fasta", config["output_folder"] + "gatk/extract_indels/" + config["read_name"] + "_raw_indels.vcf"
    output: config["output_folder"] + "gatk/filter_indels/" + config["read_name"] + "_filtered_indels.vcf"
    shell: "docker run -it --rm -v $(pwd):/root/source phedo/gatk -T VariantFiltration -R {input[0]} -V {input[1]}  --filterExpression 'QD < 2.0 || FS > 200.0 || ReadPosRankSum < -20.0' --filterName 'my_indel_filter' -o {output} "

rule gatk_combine_variants:
    input: "reference/Homo_sapiens_assembly38.fasta", config["output_folder"] + "gatk/filter_snp/" + config["read_name"] + "_filtered_snps.vcf", config["output_folder"] + "gatk/filter_indels/" + config["read_name"] + "_filtered_indels.vcf"
    output: config["output_folder"] + "gatk/combine_variants" + config["read_name"] + "_selected_variants.vcf"
    shell: "docker run -it --rm -v $(pwd):/root/source phedo/gatk -T CombineVariants -R {input[0]} --variant {input[1]} --variant {input[2]} --genotypemergeoption UNSORTED -o {output}"

rule bedtools:
    input: "reference/trusight_cardiomyopathy_manifest_a_hg38.bed", config["output_folder"] + "gatk/combine_variants" + config["read_name"] + "_selected_variants.vcf"
